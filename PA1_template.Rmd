...
Now we can use that the column 'date' was taken as a factor, in order to use it into the 'tapply' function. And then we can make the requested plot.  
```{r}
...
We will note that this mean will be truncated to make it *integer*, so will be greater errors than other approaches as using the mean of this day, or mean of this interval.
...
Reporting new 'mean' and 'median'. (Note that remove NAs is not needed).   
...
```
And a another new column that transform weekend days (0=Sunday and 6=Saturday) to "weekend", or "weekday" the other days. Then, transforms into a factor.  
```{r}
